...
public class Payment extends JPanel implements MouseListener, ActionListener {
    ...
    private void addProductList() {
        ...
        for (int i = 0; i < products.size(); i++) {
            ...
            {
                 lblPrice = new JLabel(WinkelApplication.CURRENCY + WinkelApplication.getInstance().currencyFormat.format(product.getPrice() - ((product.getDiscount() / 100) * product.getPrice())));
            }
            else
            {
                lblPrice = new JLabel(WinkelApplication.CURRENCY + WinkelApplication.getInstance().currencyFormat.format(product.getPrice()));
            }
        ...
        JLabel lblTotalPrice = new JLabel(WinkelApplication.CURRENCY + WinkelApplication.getInstance().currencyFormat.format(WinkelApplication.getBasket().getTotalCosts()));
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        WinkelApplication.getQueryManager().setOrder(WinkelApplication.getBasket(),
                naam, adres, postcode, woonplaats, betaalmethode);
